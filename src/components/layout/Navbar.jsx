  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/10 bg-black/80 backdrop-blur-2xl">
        <div className="max-w-screen-2xl mx-auto px-6 py-5 flex items-center">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <div className="relative flex items-center bg-white/10 rounded-3xl p-1 border border-white/10">
              {["en", "ru", "ua"].map((lng) => (
                <button
                  key={lng}
                  onClick={() => changeLanguage(lng)}
                  className={`relative z-10 px-6 py-2 text-xs font-medium rounded-3xl transition-all duration-200 ${
              ...
              <div
                className="absolute top-1 bottom-1 bg-white rounded-3xl shadow-inner transition-all duration-300 ease-out"
                style={{
                  left: i18nInstance.language === "en" ? "4px" : i18nInstance.language === "ru" ? "calc(33.333% + 4px)" : "calc(66.666% + 8px)",
